...
import { getLikedContentData, removeFromList } from "../utils/Functions";
...
export const getUsersLikedMovies = createAsyncThunk("netflix/getLiked", async (email) => {
    const movies = await getLikedContentData(email);
    return movies;
});
...
export const removeMovieFromLiked = createAsyncThunk("netflix/deleteLiked", async ({ movieId, email }) => {
    const movies = await removeFromList(movieId, email);
    return movies;
});
